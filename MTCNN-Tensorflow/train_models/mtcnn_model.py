...
def P_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with slim.arg_scope([slim.conv2d],
                        ...
                        padding='valid'):#给list_ops中的内容设置默认值
...
def O_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    with slim.arg_scope([slim.conv2d],
		...
		#activation_fn：激活函数选择prelu
		#weights_initializer权值初始化
		#biases_initializer偏置值初始化
		#weights_regularizer权值L2正则化
		#padding填充有效
        print(inputs.get_shape())#显示输入形状
