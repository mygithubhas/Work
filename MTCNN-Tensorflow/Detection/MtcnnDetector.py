...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        #初始化三个网络
        self.min_face_size = min_face_size#24
        self.stride = stride
    ...
    def processed_image(self, img, scale):
        ...
        '''
        height, width, channels = img.shape#图片形状
        new_height = int(height * scale)  # resized new height
        new_width = int(width * scale)  # resized new width
        new_dim = (new_width, new_height)
        img_resized = cv2.resize(img, new_dim, interpolation=cv2.INTER_LINEAR)  # resized image双线性插值（默认设置）
        # don't understand this operation
        img_resized = (img_resized - 127.5) / 128#归一化
        return img_resized
    ...
    def detect_pnet(self, im):
        ...
        current_height, current_width, _ = im_resized.shape#之后的形状
    ...
    def detect(self, img):
        ...
        # pnet
        t1 = 0
        if self.pnet_detector:#传入的Pnet为真
